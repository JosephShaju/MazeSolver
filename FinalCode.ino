...
// sensorType 1 is the Accelerometer sensor
  ...
  deltaL();//deltaL calculation
  
  virtleglen();//Virtual leg length
  
  ABC();// A,B,C values 
  
  alpharad();// Angle movement of Servo in Radians
 
  alphadeg();// Angle movement of Servo in Degrees
 
  finalval();// Final step value after Mapping
  
  set_PWM();//Setting Motor values to steps

}

void contain(){ // Function to contain the error in Acceleration
...
{
  for (i = 0; i<6 ; i++){ 
    Px[i] = ((Pox[i])*(cos(tr))*(cos(ty))) + ((Poy[i])*((sin(tp)*sin(tr)*cos(tr)) - ((cos(tp))*(sin(ty)))));
    Py[i] = (Pox[i]*(cos(tr))*(sin(ty))) + ((Poy[i])*(((cos(tp))*(cos(ty))) + ((sin(tp))*(sin(tr))*(sin(ty)))));
    Pz[i] = (-(Pox[i])*(sin(tr))) + ((Poy[i])*(sin(tp))*(cos(tr))) + ht;
  }   
}
void deltaL()
{
...
//Created seperate Inverse Sin,Cos and Tan functions to handle Infinity error
float sinin(float c){
